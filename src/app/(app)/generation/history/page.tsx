  ...
  return <main className="mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
    ...
    <section className="mt-7 space-y-3">
      {loading ? <div className="rounded-3xl border border-white/10 p-10 text-center text-zinc-500">Cargando historial…</div> : items.length === 0 ? <div className="rounded-3xl border border-white/10 p-10 text-center text-zinc-500">Todavía no hay trabajos.</div> : items.map((item)=><article key={item.id} className="rounded-3xl border border-white/10 bg-white/[.025] p-5">
        ...
        {item.tokens_charged>0&&<p className="mt-4 text-xs text-zinc-500">{item.tokens_refunded?`${item.tokens_charged} tokens reembolsados`:`${item.tokens_charged} tokens cobrados`}</p>}{item.logs?.length>0&&<p className="mt-4 rounded-xl border border-white/5 bg-black/20 p-3 text-xs text-zinc-400">Último evento: {item.logs[item.logs.length-1]?.message}</p>}{item.error&&<p className="mt-4 rounded-xl border border-red-500/20 bg-red-500/5 p-3 text-xs text-red-300">{item.error}</p>}
