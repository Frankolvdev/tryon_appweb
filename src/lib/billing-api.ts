...
export const listPlans = () => apiFetch<SubscriptionPlan[]>("/api/v1/subscription-plans");
...
export const openCustomerPortal = () => apiFetch<{ portal_url: string }>("/api/v1/billing/customer-portal", {
  method: "POST",
  body: JSON.stringify({ return_url: currentUrl("/billing") }),
});

export const cancelSubscription = () => apiFetch<{ message: string }>("/api/v1/billing/subscriptions/cancel", {
  method: "POST",
  body: JSON.stringify({ cancel_immediately: false }),
});

export const reactivateSubscription = () => apiFetch<{ message: string }>("/api/v1/billing/subscriptions/reactivate", {
  method: "POST",
  body: JSON.stringify({ confirm: true }),
});

export const synchronizeSubscription = () => apiFetch<{ message: string }>("/api/v1/billing/subscriptions/sync", { method: "POST" });

export const validateCoupon = (code: string, purchaseAmount: number, purchaseType: "plan" | "token_package", itemId: number) =>
  apiFetch<CouponValidation>("/api/v1/billing-coupons/validate", {
    method: "POST",
    body: JSON.stringify({ code, purchase_amount: purchaseAmount, purchase_type: purchaseType, item_id: itemId }),
  });
