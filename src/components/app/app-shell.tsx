  ...
  return (
    <div className="appFrame">
      <header className="mobileHeader">
        <Brand />
        <button className="menuButton" onClick={() => setOpen((value) => !value)} aria-label="Abrir navegación" aria-expanded={open}>
          <span />
          <span />
          <span />
        </button>
      ...
      {open && <button className="sidebarBackdrop" onClick={() => setOpen(false)} aria-label="Cerrar navegación" />}
      ...
      <aside className={`sidebar${open ? " sidebarOpen" : ""}`}>
        ...
        <div className="sidebarBottom">
          <Link href="/settings" className="userMiniCard" onClick={() => setOpen(false)}>
          ...
          <button className="logout" onClick={logout}>↗ <span>Cerrar sesión</span></button>
        </div>
      </aside>
