...
export function ModelStudio({modelId}:{modelId:number}){
 ...
 const preview=useMemo(()=>({
  ...
  height:nearestAsset(assets.height??[],(body.height+5)/10), bubble_butt:nearestAsset(assets.bubble_butt??[],body.bubbleButt/.7), waist:nearestAsset(assets.waist??[],(body.waist+4)/7),
 ...
 async function continueIdentity(){const identityName=isTemporaryAiModelName(displayName)?"":displayName.trim();if(!identityName){toast.error("Escribe el nombre de la modelo antes de continuar.");return}if(identityMode==="existing"&&!identityFile){toast.error("Sube un rostro antes de continuar.");return}setSaving(true);try{const current=(model?.draft_json&&typeof model.draft_json==="object")?model.draft_json:{};const updated=await saveAiModelDraft(modelId,{...current,identityMode,existingIdentityFile:identityMode==="existing"?identityFile:null,modelSetup:{sex:"woman",identityMode,existingIdentityFile:identityMode==="existing"?identityFile:null,completed:true}},identityName);setModel(updated);router.push(`/models/${modelId}/face`)}catch(e){toast.error(e instanceof Error?e.message:"No se pudo guardar Identidad")}finally{setSaving(false)}}
...
function BodySetup({body,setBody,curvy,setCurvy,preview,onContinue,saving}:{body:BodyControlState;setBody:React.Dispatch<React.SetStateAction<BodyControlState>>;curvy:boolean;setCurvy:(v:boolean)=>void;preview:Record<string,ModelGenerationAsset|null>;onContinue:()=>void;saving:boolean}){
 ...
 <Control label="Waist" value={body.waist} display={body.waist>0?`+${body.waist}`:`${body.waist}`} min={-4} max={3} step={1} left="-4" right="+3" asset={preview.waist} onChange={v=>set("waist",v)}/>
 </div><button className="modelConfirm" onClick={onContinue} disabled={saving}><Check size={17}/>{saving?"Guardando…":"Continuar"}</button></section>
}
