...
export function ModelStudio({modelId}:{modelId:number}){
 ...
 async function continueIdentity(){const identityName=displayName.trim()==="Nueva modelo"?"":displayName.trim();if(!identityName){toast.error("Escribe el nombre de la modelo antes de continuar.");return}if(identityMode==="existing"&&!identityFile){toast.error("Sube un rostro antes de continuar.");return}setSaving(true);try{const current=(model?.draft_json&&typeof model.draft_json==="object")?model.draft_json:{};const updated=await saveAiModelDraft(modelId,{...current,modelSetup:{sex:"woman",identityMode,existingIdentityFile:identityMode==="existing"?identityFile:null,completed:true}},identityName);setModel(updated);router.push(`/models/${modelId}/face`)}catch(e){toast.error(e instanceof Error?e.message:"No se pudo guardar Identidad")}finally{setSaving(false)}}
